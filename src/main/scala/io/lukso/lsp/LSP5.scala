...
trait LSP5:
  /** Read all received asset contract addresses from a UP's LSP5ReceivedAssets[] array. */
  def getReceivedAssets(upAddress: String): IO[RpcError, List[String]]

object LSP5:
  def getReceivedAssets(upAddress: String): ZIO[LSP5, RpcError, List[String]] =
    ZIO.serviceWithZIO(_.getReceivedAssets(upAddress))

  val layer: ZLayer[ERC725Y, Nothing, LSP5] =
    ZLayer.fromFunction { (erc725y: ERC725Y) =>
      new LSP5:
        override def getReceivedAssets(upAddress: String): IO[RpcError, List[String]] =
          for
            lengthHex <- erc725y.getData(upAddress, DataKeys.LSP5ReceivedAssets)
            count      = parseUint128Length(lengthHex)
            indexKeys  = (0 until count).toList.map(i => DataKeys.arrayElementKey(DataKeys.LSP5ReceivedAssets, i))
